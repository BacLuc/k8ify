...
func createServiceMonitorEndpoint(name string, servicePorts []core.ServicePort, config *util.ServiceMonitorConfig, labels map[string]string) (prometheusTypes.Endpoint, []core.Secret) {
...
func endpointPointer(config *util.ServiceMonitorConfig, servicePorts []core.ServicePort) prometheusTypes.Endpoint {
...
func createBasicAuthForEndpoint(secretName string, labels map[string]string) (*prometheusTypes.BasicAuth, map[string]string) {
	basicAuthConfig := util.ServiceMonitorBasicAuthConfigPointer(labels)
...
func createTlsConfigForEndpoint(secretName string, labels map[string]string) (*prometheusTypes.TLSConfig, map[string]string) {
	labelTlsConfig := util.ServiceMonitorTlsConfigPointer(labels)
	if !labelTlsConfig.Enabled {
		return nil, map[string]string{}
	}

	safeTlsConfig := prometheusTypes.SafeTLSConfig{
		InsecureSkipVerify: labelTlsConfig.InsecureSkipVerify,
		MaxVersion:         parseTlsVersion(labelTlsConfig.MaxVersion),
		MinVersion:         parseTlsVersion(labelTlsConfig.MinVersion),
	...
	if labelTlsConfig.Ca != nil {
		...
		safeTlsConfig.CA = prometheusTypes.SecretOrConfigMap{Secret: getPointer(secretKeySelector(secretName, secretKey))}
	}

	if labelTlsConfig.Cert != nil {
		secretKey := "cert"
		secretEntries[secretKey] = *labelTlsConfig.Cert
		safeTlsConfig.Cert = prometheusTypes.SecretOrConfigMap{Secret: getPointer(secretKeySelector(secretName, secretKey))}
	}

	if labelTlsConfig.KeySecretValue != nil {
		secretKey := "key"
		secretEntries[secretKey] = *labelTlsConfig.KeySecretValue
		safeTlsConfig.KeySecret = getPointer(secretKeySelector(secretName, secretKey))
	}

	return getPointer(prometheusTypes.TLSConfig{
...
func parseTlsVersion(string *string) *prometheusTypes.TLSVersion {
	if string == nil {
		return nil
	}
	switch *string {
	case tlsVersion10:
		return getPointer(prometheusTypes.TLSVersion10)
	case tlsVersion11:
		return getPointer(prometheusTypes.TLSVersion11)
	case tlsVersion12:
		return getPointer(prometheusTypes.TLSVersion12)
	case tlsVersion13:
		return getPointer(prometheusTypes.TLSVersion13)
	default:
		panic(fmt.Sprintf("unknown TLSVersion: %v", *string))
	}
}
